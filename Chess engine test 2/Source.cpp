...
struct ChessBoard {
	enum class Turn { white, black } turn; //turn system
	enum class Piece { king, queen, white_pawn, black_pawn, rook, bishop, knight }; //7 possible different pieces
	static map<Piece, int> pieceValues; //each piece has different numerical value
...
int main() {
	ChessBoard game;
	cout << endl << "# Prototype Chess Engine" << endl;
	
	game.printHelp();
	bool game_on = true;
	while (game_on) {
		game.reset();
		string pick_side = ""; //by default pick_side is empty
		while (pick_side != "b" && pick_side != "w" && pick_side != "q") {
			cout << endl << " Want to Play as a black or white? | b = black, w = white, q = quit: ";
			cin >> pick_side;
		}
		if (pick_side == "q") {
		...
		while (game_on = game.promptInput()) {
			if (!game.hasKing()) {
				cout << "* You won!" << endl;
				break;
			}
